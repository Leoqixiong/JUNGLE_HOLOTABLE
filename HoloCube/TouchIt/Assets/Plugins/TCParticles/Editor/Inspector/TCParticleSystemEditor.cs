...
namespace TC.EditorIntegration {
	...
	public class TCParticleSystemEditor : TCEdtiorBase<TCParticleSystem> {
		...
		Dictionary<TCParticleSystem, TCWireframeDrawer> m_drawers = new Dictionary<TCParticleSystem, TCWireframeDrawer>();
			...
			if (GUI.changed) {
				...
				foreach (var system in Targets) {
					if (system.Emitter.Shape == EmitShapes.Mesh) {
						m_drawers[system].UpdateMesh((system.Emitter).EmitMesh);
					}
				}
			}
		}
		...
		void OnSceneGUI() {
			...
			TCDrawFunctions.DrawEmitterShape(emitter.ShapeData, transform, drawer);
		}
