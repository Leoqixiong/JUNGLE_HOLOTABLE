...
namespace TC {
	...
	public class ParticleEmitter : ParticleComponent {
		...
		protected override void Bind() {
			...
			SetShapeData(pes, SystemComp.transform, ref m_emitPrevPos, ref m_emitPrevSpeed);
		}
		...
		internal void UpdatePlayEvent(float prevSystemTime) {
			...
			if (DoEmit && m_emitTag != null) {
				for (int i = 0; i < Tracker<TCShapeEmitter>.Count; i++) {
					...
					int num = emitter.TickEmission(SystemComp);
					...
					//pdate shape
					SetShapeData(emitter.ShapeData, emitter.transform, ref emitter.PrevPos, ref emitter.PrevSpeed);
		...
		//TODO: Add usePosition for when people only want to set eg color
		...
		public void DispatchEmitExtensionKernel(ComputeShader cs, int kernel) {
			...
			cs.SetFloat(SID._ParticleEmitCount, m_currentEmitBind.Count);
